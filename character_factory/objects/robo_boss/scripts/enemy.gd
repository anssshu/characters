...
#get the player
onready var player = get_tree().root.get_node("boss_fight/player")
...
func handle_player_facing():
	...
	if v.x < 0:
		face = "left"
	#boss face towards the player always	
...
func return_random_states(state_list):
	return state_list[randi()%len(state_list)]
